    ...
    <div class="card-header reply-header">
        {{$reply->created_at->diffForHumans()}} by <a href="#">{{$reply->user->name}}</a>
            ...
            <form method="POST" action="{{route('favorite_reply', ['reply' => $reply->id])}}" id="submit_like_reply">
                {{csrf_field()}}
                <button type="submit" class="like-reply" @if($reply->isFavorited()) disabled @endif>
                    @if ($reply->favorites()->count() > 0)
                        <i class="fa fa-thumbs-o-up liked" aria-hidden="true"></i><span class="likes-count">{{$reply->favorites()->count()}}</span>
                    @else
